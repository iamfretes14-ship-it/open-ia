...
impl ToolEntry {
    fn new(name: String, info: ToolInfo) -> Self {
        let input_keys = collect_input_keys(info.tool.input_schema.as_ref());
...
fn collect_input_keys(input_schema: &serde_json::Map<String, JsonValue>) -> Vec<String> {
    let Some(JsonValue::Object(map)) = input_schema.get("properties") else {
        return Vec::new();
    };
